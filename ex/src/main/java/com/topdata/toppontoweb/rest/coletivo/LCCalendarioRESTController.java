...
import com.topdata.toppontoweb.services.coletivo.LCCalencarioServices;
import org.springframework.transaction.annotation.Transactional;
//</editor-fold>

/**
 * Classe de acesso aos métodos REST para o lançamento coletivo do calendário
 *
 ...
 */
@Path("/coletivo/calendario")
@Singleton
@Autowire
public class LCCalendarioRESTController implements ColetivoRestInterface {

    @Autowired
    private LCCalencarioServices lCCalencarioServices;
    ...
    public Response salvar(@Context HttpServletRequest request, ColetivoTransfer coletivoTransfer) {
        ProgressoTransfer progresso = lCCalencarioServices.criarProgresso();
        ...
        return lCCalencarioServices.getTopPontoResponse().sucessoSalvar(progresso);
    }
   
    private void doSalvar(@Context HttpServletRequest request, ColetivoTransfer coletivoTransfer, ProgressoTransfer progresso) {
        try {
            lCCalencarioServices.salvarColetivo(request, coletivoTransfer, progresso);
    ...
    public Response atualizar(@Context HttpServletRequest request, ColetivoTransfer coletivoTransfer) {
        ...
        return lCCalencarioServices.getTopPontoResponse().sucessoAtualizar(progresso);
    }
    

    public void doAtualizar(@Context HttpServletRequest request, ColetivoTransfer coletivoTransfer, ProgressoTransfer progresso) {
        try {
            lCCalencarioServices.atualizarColetivo(request, coletivoTransfer, progresso);
        } catch (ServiceException ex) {
            throw new WebApplicationException(ex.getResponse());
        }
    }
    ...
    public List<ColetivoTransfer> listar(@Context HttpServletRequest request) {
        try {
            return lCCalencarioServices.listarColetivos(request);
        } catch (ServiceException ex) {
            throw new WebApplicationException(ex.getResponse());
        }
    }
    ...
    public Response buscarProgresso(@Context HttpServletRequest request, @PathParam("id") long id) {
        final ProgressoTransfer progresso = lCCalencarioServices.buscarProgresso(id);
        ...
        if(progresso.getColetivoResultados() != null && progresso.getColetivoResultados() .size() > 0 ){
            lCCalencarioServices.removerProgresso(id);
        }
    ...
    public ColetivoTransfer listar(@Context HttpServletRequest request, @PathParam("id") Integer id) {
        try {
            return lCCalencarioServices.buscarColetivo(request, id);
        } catch (ServiceException ex) {
            throw new WebApplicationException(ex.getResponse());
        }
    }
    ...
    public Response excluir(@Context HttpServletRequest request, @PathParam("id") Integer id) {
        try {
            return lCCalencarioServices.excluirColetivo(request, id);
        } catch (ServiceException ex) {
            throw new WebApplicationException(ex.getResponse());
        }
    }
