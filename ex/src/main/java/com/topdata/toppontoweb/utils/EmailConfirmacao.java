...
public class EmailConfirmacao {
    ...
    @Value("${email.confirmacao}")
    private String Link;
    @Value("${email.assunto.confirmacao}")
    ...
    public void enviarEmail(HttpServletRequest request, Operador operador) throws ServiceException {
        ...
        try {
            ...
            String msg = "Prezado " + nome + ",\n"
                    + "\n"
                    + "Seu cadastro foi efetuado com sucesso no sistema TopPonto. Clique no link " + linkFinal + operador.getCodigoConfirmacaoNovaSenha() + " para cadastrar sua senha.\n"
                    + "\n"
                    + "Atenciosamente,\n"
