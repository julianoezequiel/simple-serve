 ...
 */
@Repository
public class FechamentoTabelaExtrasDSRDao extends Dao<FechamentoTabelaExtrasDSR, Integer> {

    @Transactional
    public long excluirPorEmpresaFechamentoData(EmpresaFechamentoData efd) throws ServiceException {
        try {
            Query query;
            query = this.getEntityManager().createQuery("DELETE FROM FechamentoTabelaExtras fta WHERE fta.idEmpresaFechamentoData =:efd");
